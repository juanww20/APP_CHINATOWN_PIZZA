...
import {MenuSectionButton, MenuProduct} from '../components/menu'
...
export default function Menu_general({navigation, cart, setCart}) {
        ...
        return (
            <View style={styles.contenedor}>
                ...
                <FlatList
                    data={data}
                    renderItem={({ item }) => <MenuProduct item={item} cart={cart} setCart={setCart}/>}
                    keyExtractor={item => item.id}
                    style={{height: '70%'}}
                    showsVerticalScrollIndicator={false}
                />

                <CasualButton texto="Ver Orden" func={() => navigation.navigate('Carrito')} styles={{height: '10%'}}></CasualButton>
            </View>
        )
    }
